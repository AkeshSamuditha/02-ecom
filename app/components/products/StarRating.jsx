...
const StarRating = ({ product }) => {
  ...
  return ratingDiff ? (
    <>
      {new Array(Math.floor(product?.rating ?? 5)).fill().map((_) => (
        <GiRoundStar key={_} className=" text-yellow-400 mb-1" />
      ))}
    ...
    new Array(Math.floor(product?.rating ?? 5))
      .fill()
      .map((i) => <GiRoundStar key={_} className=" text-yellow-400 mb-1" />)
  );
};
